...
-- PIVOT TABLE
-- Dynamic SQL QUERY
-- it does not matter if the fruits multiply
DECLARE @tx varchar(500)
DECLARE  @sql varchar(300) ;
SELECT
       @tx=COALESCE(@tx+',['+Fruit+']','['+Fruit+']')
FROM
      ( SELECT DISTINCT Fruit FROM tblFruit) AS T
SET @sql = 'SELECT * FROM tblFruit PIVOT ( count(Fruit) FOR Fruit in ('+@tx+') ) as p'
EXECUTE (@sql);
-- #################################
...
SELECT * FROM tblFruit
     PIVOT ( count(Fruit) FOR Fruit in ([APPLE],[LICHI],[MANGO],[ORANGE]) ) as p
-- ###########################

-- CASE 4 #####################
-- JOIN not a good solution
-- almost everyone will be by group, but everyone comes from a different table
